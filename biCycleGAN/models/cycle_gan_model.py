...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.input_BC = self.Tensor(nb, opt.input_nc, size, size)
        ...
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        self.netG_BC = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        self.netG_C = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.netD_C = networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            self.netD_BC = networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
        if not self.isTrain or opt.continue_train:
            ...
            if self.isTrain:
                ...
                self.load_network(self.netD_BC, 'D_BC', which_epoch)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters(), self.netG_BC.parameters(), self.netG_C.parameters()),
    ...
    def set_input(self, input):
        ...
        BtoC = self.opt.which_direction == 'BtoC'
        input_B = input['B' if BtoC else 'C']
        input_C = input['C' if BtoC else 'B']
        self.input_B.resize_(input_B.size()).copy_(input_B)
        self.input_C.resize_(input_C.size()).copy_(input_C)
        self.image_paths = input['B_paths' if BtoC else 'C_paths']
    ...
    def test(self):
        ...
        self.rec_BC = self.netG_C(fake_C).data
        self.fake_C = fake_C.data

        real_C = Variable(self.input_C, volatile=True)
        fake_B = self.netG_C(real_C)
        self.rec_C = self.netG_BC(fake_B).data
        self.fake_B = fake_B.data
    ...
    def backward_D_BC(self):
        ...
        loss_D_BC = self.backward_D_basic(self.netD_BC, self.real_C, fake_C)
        self.loss_D_BC = loss_D_BC.data[0]

    def backward_D_C(self):
        fake_B = self.fake_B_pool.query(self.fake_B)
        loss_D_C = self.backward_D_basic(self.netD_C, self.real_B, fake_B)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            loss_idt_BC = self.criterionIdt(idt_BC, self.real_C) * lambda_B * lambda_idt
            
            idt_C = self.netG_C(self.real_C)
            loss_idt_C = self.criterionIdt(idt_C, self.real_B) * lambda_A * lambda_idt
        ...
        self.rec_BC = rec_B.data
        self.rec_C = rec_B.data
    ...
    def get_current_visuals(self):
        ...
        rec_BC = util.tensor2im(self.rec_BC)
        ...
        ret_visuals = OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('rec_A', rec_A),
                                   ...
                                   ('real_B', real_B), ('fake_C', fake_C), ('rec_BC', rec_BC),
                                   ('real_C', real_C), ('fake_B', fake_B), ('rec_C', rec_C),])
    ...
    def save(self, label):
        ...
        self.save_network(self.netD_C, 'D_BC', label, self.gpu_ids)