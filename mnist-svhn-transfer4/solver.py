...
from model import G12, G21 , G32, G23
from model import D3, D2, D1
...
class Solver(object):
    ...
    def build_model(self):
        ...
        g_params = list(self.g12.parameters()) + list(self.g21.parameters()) + list(self.g32.parameters()) + list(self.g23.parameters())
        d_params = list(self.d3.parameters()) + list(self.d2.parameters()) + list(self.d1.parameters())
    ...
    def train(self):
        ...
        for step in range(self.train_iters+1):
            ...
            d_mnist_loss = d3_loss + d1_loss
            d_svhn_loss = d2_loss
            d_real_loss = d3_loss + d2_loss + d1_loss
            ...
            fake_mnist = self.g23(svhn)
            out = self.d3(fake_mnist)
            if self.use_labels:
                d3_loss = criterion(out, mnist_fake_labels)
            else:
                d3_loss = torch.mean(out**2)

            fake_svhn = self.g32(-mnist)
            out = self.d2(fake_svhn)
            if self.use_labels:
                d2_loss += criterion(out, svhn_fake_labels)
            else:
            ...
            d_fake_loss = d3_loss + d2_loss +d1_loss
            ...
            fake_svhn = self.g32(-mnist)
            out = self.d2(fake_svhn)
            reconst_mnist = self.g23(fake_svhn)
            if self.use_labels:
                g_loss = criterion(out, m_labels) 
            else:
                g_loss = torch.mean((out-1)**2) 
            ...
            fake_mnist = self.g23(svhn)
            out = self.d3(fake_mnist)
            reconst_svhn = self.g32(fake_mnist)
            if self.use_labels:
            ...
            if (step+1) % self.sample_step == 0:
                ...
                fake_svhn = self.g32(-fixed_mnist)
                fake_mnist = self.g23(fixed_svhn)
                
                mnist, fake_mnist = self.to_data(-fixed_mnist), self.to_data(fake_mnist)
                svhn , fake_svhn = self.to_data(fixed_svhn), self.to_data(fake_svhn)
                
                merged = self.merge_images(mnist, fake_svhn)
                path = os.path.join(self.sample_path, 'sample2-%d-m-s.png' %(step+1))
                scipy.misc.imsave(path, merged)
                print ('saved %s' %path)
                
                merged = self.merge_images(svhn, fake_mnist)
                path = os.path.join(self.sample_path, 'sample2-%d-s-m.png' %(step+1))
                scipy.misc.imsave(path, merged)
                print ('saved %s' %path)
            ...
            if (step+1) % 5000 == 0:
                ...
                g32_path = os.path.join(self.model_path, 'g32-%d.pkl' %(step+1))
                g23_path = os.path.join(self.model_path, 'g23-%d.pkl' %(step+1))
                d3_path = os.path.join(self.model_path, 'd3-%d.pkl' %(step+1))
                ...
                torch.save(self.g32.state_dict(), g32_path)
                torch.save(self.g23.state_dict(), g23_path)
                torch.save(self.d3.state_dict(), d3_path)
